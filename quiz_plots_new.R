...
d.survey$Date <- as.Date(d.survey$Submission.Date, format = "%Y-%m-%d")

no.resp <- as.data.frame(table(d.survey$Enter.login.ID))
resp.perday <- as.data.frame(table(d.survey$Date))
resp.peridperday <- as.data.frame.matrix(table(d.survey$Enter.login.ID, d.survey$Date))

respq1 <- as.data.frame(table(d.survey$Are.you.indoors.or.outdoors.))
respq2 <- as.data.frame(table(d.survey$Not.including.cigarette.smoking..can.you.smell.smoke.))
respq3 <- as.data.frame(table(d.survey$How.does.the.air.outside.look.to.you.))
respq2q3 <- as.data.frame.matrix(table(d.survey$How.does.the.air.outside.look.to.you.,
                                d.survey$Not.including.cigarette.smoking..can.you.smell.smoke.))
respq4 <- as.data.frame(table(d.survey$RIGHT.NOW..do.you.have.irritated.eyes..nose..throat.))
respq4.id <- as.data.frame.matrix(table(d.survey$Enter.login.ID,
                                 d.survey$RIGHT.NOW..do.you.have.irritated.eyes..nose..throat.))
respq5.id <- as.data.frame.matrix(table(d.survey$Enter.login.ID,
                                 d.survey$RIGHT.NOW..do.you.have.wheezing.))

respq13.id <- as.data.frame.matrix(table(d.survey$Enter.login.ID,
                                         d.survey$RIGHT.NOW..do.you.have.a.cough.))

respq14.id <- as.data.frame.matrix(table(d.survey$Enter.login.ID,
                    d.survey$How.would.you.rate.air.quality.right.now.))

respq14.date <- as.data.frame.matrix(table(d.survey$Date,
                                           d.survey$How.would.you.rate.air.quality.right.now.))
respq14.date <- as.data.frame.matrix(table(d.survey$Date,
                                           d.survey$Not.including.cigarette.smoking..can.you.smell.smoke.))


## total number of health symptoms:
symptoms <-  d.survey %>% group_by(Date) %>%
  dplyr::summarise(breathlessness = mean(as.numeric(RIGHT.NOW..do.you.have.breathlessness.), na.rm = T),
                   irritated = mean(as.numeric(RIGHT.NOW..do.you.have.irritated.eyes..nose..throat.), na.rm = T),
                   asthma = mean(as.numeric(Have.you.had.to.use.an.inhaler.or.other.asthma.medicine.in.the.last.hour.), na.rm = T),
                   cough = mean(as.numeric(RIGHT.NOW..do.you.have.a.cough.), na.rm = T))

ggplot(symptoms) +
  geom_col(aes(Date,breathlessness))
...
ggplot(d.survey[which(d.survey$How.would.you.rate.air.quality.right.now. != ""),]) +
  geom_bar(aes(How.would.you.rate.air.quality.right.now.), fill = "darkgreen") + theme_bw()+
...
######quiz questions plots

# PDFfile <- paste0(path,"Response to quiz questions.pdf")
# pdf(file=PDFfile, paper = "USr", width = 22)

## q1
ggplot(d.survey[which(d.survey$X.Banking...filling.your.fireplace.with.wood.and.turning.down.low.before.going.to.bed.. != ""),]) +
  geom_bar(aes(X.Banking...filling.your.fireplace.with.wood.and.turning.down.low.before.going.to.bed..), fill = "darkred") + theme_bw()+
  ggtitle("Banking...") +xlab(" ") +
  theme(axis.title = element_text(size = 22), 
        axis.text = element_text(size = 22),
        title = element_text(size = 22))
